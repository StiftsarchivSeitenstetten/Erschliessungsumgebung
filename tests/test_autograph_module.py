...
class AutographModuleTest(unittest.TestCase):
    ...
    def test_module_schema_core_types_repeaters_and_presets_are_declarative(self):
        ...
        self.assertEqual([option["value"] for option in repeater.item_fields[0].options], ["person", "organization", "family"])
    ...
    def test_minimal_full_multiple_agents_dates_and_optional_end_validate(self):
        ...
        self.runtime.validate(self.create(open_end, operation_id="autograph-create-3").data)
    ...
    def test_internal_field_is_filtered_and_server_side_protected(self):
        ...
        for role in ("redaktion", "admin"):
            self.assertEqual(self.runtime.filter_for_view(stored.data, role)["erschliessung"]["interne_bemerkung"], "Nur intern.")
            self.assertEqual(
                self.runtime.filter_for_edit({"erschliessung": {"interne_bemerkung": "Geändert"}}, role),
                {"erschliessung": {"interne_bemerkung": "Geändert"}},
            )
        with self.assertRaises(RecordPermissionError):
            self.runtime.filter_for_edit({"erschliessung": {"interne_bemerkung": "Manipulation"}}, "ehrenamtlich")
    ...
    def test_create_update_revision_order_and_search_use_generic_persistence(self):
        ...
        self.assertEqual([item["agent"]["name"] for item in stored.data["erschliessung"]["beteiligte"]],
                         ["Max Mustermann", "Stift Seitenstetten"])
        self.assertEqual(set(self.repository.commits[-1]["files"]), {
        ...
        updated = update_generic_record(
            self.repository, self.module, stored.record_id, payload, stored.revision, self.editor,
        )
        self.assertNotEqual(updated.revision, stored.revision)
        self.assertEqual(parse_record_content(self.repository.files[updated.path])["erschliessung"]["regest"], "Geändertes Regest")
        index = read_module_index(self.repository, self.module)
        for query in ("9.6.1", "Mustermann", "Brief", "Wien", "Geändertes", "Autogr. 42"):
            self.assertEqual([row["record_id"] for row in query_index(
                self.module, index, "ehrenamtlich", q=query,
            )], [stored.record_id])
        self.assertEqual(query_index(
            self.module, index, "ehrenamtlich", q="Nur intern",
        ), [])
    ...
    def test_module_descriptor_does_not_expose_internal_field_to_volunteer(self):
        ...
        internal_volunteer = next(field for field in volunteer["fields"] if field["path"] == "erschliessung.interne_bemerkung")
        internal_editor = next(field for field in editor["fields"] if field["path"] == "erschliessung.interne_bemerkung")
        ...
        self.assertNotIn("erschliessung.interne_bemerkung", volunteer["search"]["fulltext"])
    ...
    def test_generic_renderer_handles_nested_autograph_fields(self):
        ...
        subprocess.run([node, "tests/autograph_form.mjs"], cwd=ROOT, check=True, capture_output=True)
...
class AutographApiTest(unittest.TestCase):
    ...
    def login(self, role, username=None):
        ...
        with SessionLocal() as db:
            create_user(
                db, username=username, display_name=username, email=f"{username}@example.test",
                role=role, ui_profile="ehrenamt-standard" if role == "ehrenamtlich" else "redaktion",
                modules=[MODULE_AUTOGRAPHEN_9_6], password="SehrGeheim123",
            )
            db.commit()
    ...
    def csrf(self):
        ...
        return {"X-CSRF-Token": self.client.cookies.get(name)}

    def test_suggestion_does_not_reserve_and_create_assigns_final_identity(self):
        self.login("ehrenamtlich")
        state_before = self.repository.files[self.module.storage["state"]["path"]]
        catalog_entry = next(item for item in self.client.get("/api/modules").json() if item["id"] == "autographen_9_6")
        ...
        self.assertEqual(first.json()["empty_record"]["signatur"]["anzeige"], "9.6.1")
        self.assertNotIn("id", first.json()["identity_suggestion"])
        self.assertEqual(first.json()["identity_suggestion"], second.json()["identity_suggestion"])
        self.assertEqual(self.repository.files[self.module.storage["state"]["path"]], state_before)
        response = self.client.post(
            "/api/modules/autographen_9_6/records",
            json={"operation_id": "api-create-1", "record": sample_payload(internal=False)}, headers=self.csrf(),
        )
    ...
    def test_volunteer_cannot_read_or_write_internal_field(self):
        stored = create_generic_record(
            self.repository, self.module, sample_payload(),
            type("User", (), {"username": "rita", "role": "redaktion"})(),
            self.module.create_strategy["server_values"], operation_id="seed-record",
        )
        self.login("ehrenamtlich")
        response = self.client.get(f"/api/modules/autographen_9_6/records/{stored.record_id}")
        self.assertEqual(response.status_code, 200)
        self.assertNotIn("interne_bemerkung", response.json()["record"]["erschliessung"])
        manipulated = sample_payload(internal=False)
        manipulated["erschliessung"]["interne_bemerkung"] = "Manipulation"
        denied = self.client.post(
            "/api/modules/autographen_9_6/records",
            json={"operation_id": "denied-create", "record": manipulated}, headers=self.csrf(),
        )
    ...
    def test_vocabulary_admin_lifecycle_uses_existing_api_and_stable_term_id(self):
        ...
        current = self.client.get(f"/api/vocabularies/{vocabulary_id}").json()
        added = self.client.post(
            f"/api/vocabularies/{vocabulary_id}/terms",
            json={"base_revision": current["meta"]["revision"], "term": {"id": "zeuge", "label": "Zeuge"}},
            headers=self.csrf(),
        )
        self.assertEqual(added.status_code, 201, added.text)
        payload = sample_payload()
        payload["erschliessung"]["beteiligte"][0]["rolle"] = {"id": "zeuge", "vocabulary_id": vocabulary_id}
        created = self.client.post(
            "/api/modules/autographen_9_6/records",
            json={"operation_id": "vocabulary-record", "record": payload}, headers=self.csrf(),
        )
        self.assertEqual(created.status_code, 201, created.text)
        renamed = self.client.patch(
            f"/api/vocabularies/{vocabulary_id}/terms/zeuge",
            json={"base_revision": added.json()["meta"]["revision"], "label": "Bezeugende Person"}, headers=self.csrf(),
        )
        self.assertEqual(renamed.status_code, 200, renamed.text)
        deactivated = self.client.patch(
            f"/api/vocabularies/{vocabulary_id}/terms/zeuge",
            json={"base_revision": renamed.json()["meta"]["revision"], "active": False}, headers=self.csrf(),
        )
        self.assertEqual(deactivated.status_code, 200, deactivated.text)
        term = next(item for item in deactivated.json()["vocabulary"]["terms"] if item["id"] == "zeuge")
        self.assertEqual(term["label"], "Bezeugende Person")
        self.assertFalse(term["active"])
        record = self.client.get(f"/api/modules/autographen_9_6/records/{created.json()['record_id']}").json()["record"]
        self.assertEqual(record["erschliessung"]["beteiligte"][0]["rolle"]["id"], "zeuge")
