...
class GenericFormRendererStaticTest(unittest.TestCase):
    def test_generic_shell_loads_module_descriptor_and_existing_records(self):
        ...
        self.assertIn('id="open-queue-entry"', html)
        self.assertIn('id="discard-queue-entry"', html)
    ...
    def test_form_state_builds_payload_from_editable_visible_fields_only(self):
        ...
        self.assertIn("buildEditableSnapshot", script)
        self.assertIn("loadWorkingSnapshot(snapshot)", script)
        self.assertIn("field.visible !== false", script)
    ...
    def test_generic_write_queue_is_persistent_shared_and_module_neutral(self):
        ...
        self.assertIn("initializeRecovery", queue)
        self.assertIn("retryFirst", queue)
        self.assertIn("normalizeQueueEntry", queue)
        self.assertIn("resolveUpdate", queue)
