...
Bei Validierungs- und Berechtigungsfehlern, `409`, Netzwerk- oder Serverfehlern bleiben `workingRecord`, `serverSnapshot`, `pendingSnapshot` und die bestätigte Basisrevision unverändert. Der persistierte Queue-Eintrag bleibt die Grundlage einer späteren, ausdrücklich noch nicht implementierten Wiederaufnahme. Es gibt kein automatisches Retry, Force-Save oder clientseitiges Zusammenführen.
...
Beim Anwendungsstart werden vorhandene PUT- und Create-Einträge lediglich gelesen und gemeinsam gezählt. Die Anzeige unterscheidet insbesondere `reserving`, `queued`, `saving`, `confirmed` und Fehlerzustände. Der Worker verarbeitet nur Operationen, die seine laufende Anwendungssitzung selbst neu persistiert hat; vorgefundene Einträge werden weder gelöscht noch automatisch gesendet und blockieren die FIFO-Spur. Wiederaufnahme, Retry-UI, Login-Wiederaufnahme, Read-back, unklare Create-/PUT-Auflösung, Merge und Force-Save bleiben ausdrücklich einem späteren Recovery-Schritt vorbehalten.
