...
Bei Validierungs- und Berechtigungsfehlern, `409`, Netzwerk- oder Serverfehlern bleiben `workingRecord`, `serverSnapshot` und die bestätigte Basisrevision unverändert; nur `pendingSnapshot` wird nach dem fehlgeschlagenen Versuch verworfen. Ein neuer Speicheranlauf bleibt möglich. Es gibt kein automatisches Retry, Force-Save oder clientseitiges Zusammenführen.
