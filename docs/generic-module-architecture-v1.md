...
##### Persistente PUT-Speicherwarteschlange

Updates bestehender Datensätze werden vor dem Remote-PUT lokal in der nativen IndexedDB-Datenbank `Erschliessungsumgebung`, Object Store `save_queue`, persistiert. Jeder generische Eintrag wird über eine neue `operation_id` (`crypto.randomUUID()`) identifiziert und enthält Modul, Operation `update`, Record-ID, die konkrete bestätigte Record-/Dateirevision als `base_revision`, den tief kopierten fachlichen Snapshot, FIFO-Sequenz, Zeitstempel, Status, Versuchszähler und strukturierte letzte Fehlerdaten. `pendingSnapshot` entspricht während dieses Vorgangs dem lokal gesicherten Stand; IndexedDB ist die persistente Quelle, während `FormState` ausschließlich den Zustand des aktuell geöffneten Formulars verwaltet.

Der Ablauf lautet `dirty → lokale Transaktion → queued → saving → clean/dirty`. Erst nach erfolgreichem Abschluss der IndexedDB-Transaktion darf der sequenzielle Worker den unveränderten Queue-Snapshot samt gespeicherter `base_revision` per PUT übertragen. Pro Worker läuft höchstens ein Remote-Write; FIFO wird über `queue_sequence` festgelegt. Ein Fehler stoppt die Verarbeitung vor allen späteren Einträgen. Für denselben Record wird kein zweiter Queue-Eintrag angelegt, solange der erste noch vorhanden ist.

Nach der lokalen Persistierung gilt der vorgemerkte Snapshot als navigationssicher. Nur Änderungen des `workingRecord`, die darüber hinausgehen, blockieren weiterhin die Navigation. Eine Bestätigung wird über `operation_id`, Modul, Record-ID und die konkrete Form-State-Instanz korreliert; sie darf niemals einen inzwischen geöffneten anderen Record verändern. Nach Erfolg wird der fachliche Queue-Snapshot zum bestätigten `serverSnapshot`, die neue Blob-Revision übernommen und der Queue-Eintrag entfernt. Spätere Formulareingaben bleiben `dirty`. `Gespeichert` erscheint nur bei `clean`; `queued` wird als lokale Sicherung mit ausstehender Übertragung ausgewiesen.

Bei `401/403`, `409`, `422`, 5xx oder Netzwerkfehlern bleiben Queue-Eintrag und Snapshot erhalten, `attempt_count` und `last_error` werden aktualisiert und die Verarbeitung pausiert ohne automatisches Retry. Nach bereits bestätigtem Backend-PUT wird vor dem Entfernen der Status `confirmed` persistiert; schlägt die lokale Bereinigung fehl, wird dieser Eintrag nicht blind erneut gesendet. Beim Anwendungsstart werden vorhandene Einträge lediglich gelesen und in einer globalen Anzahl angezeigt. Der Worker verarbeitet nur Operationen, die seine laufende Anwendungssitzung selbst neu persistiert hat; ein beim Reload vorgefundener erster FIFO-Eintrag bleibt pausiert und blockiert auch später neu vorgemerkte Einträge. `queued`, `saving`, `confirmed` und Fehlerzustände werden weder automatisch gelöscht noch fortgesetzt; Wiederaufnahme, Read-back und manuelles Recovery folgen separat. POST-Neuanlagen verwenden weiterhin den direkten Create-Pfad und sind nicht Teil dieser Queue.
