...
Ohne Speichern verändern Bearbeitungen weder Server noch Datenrepository. Autosave und ein produktiver Ersatz der Foto-Maske gehören weiterhin nicht zu diesem Schritt.
...
Die Endpunkte `POST /api/modules/{module_key}/records`, `POST /api/modules/{module_key}/reservations` und `PUT /api/modules/{module_key}/records/{record_id}` sind implementiert. Alle verlangen Anmeldung, Modulzugriff und CSRF-Token. Zusätzlich muss das konkrete Modul in der serverseitigen, kommaseparierten Environment-Konfiguration `GENERIC_WRITE_MODULES` stehen; der leere Default sperrt alle generischen Writes mit `503`. Tests können dieselbe Allowlist gezielt über `app.state.generic_write_modules` setzen. Ein unbekannter Eintrag erzeugt kein Modul und erteilt daher kein Recht. Die Legacy-Foto-Endpunkte unter `/api/records/photos` sind von dieser Freigabe unabhängig. Die Moduldefinition liefert `datensatz_typ`. Ein optionaler `app.state.generic_server_values_provider(module, user, payload)` kann andere schemabedingte Server-Standardwerte liefern; technische ID und Signatur werden stets durch die zentralen Strategien vergeben und überschreiben solche Vorgaben.

### Koexistenz der Browser-Queues

Die generische Moduloberfläche verwendet ausschließlich `app/generic/module-save-queue-store.js` und `app/generic/module-save-queue.js`. Ihre IndexedDB heißt `ErschliessungsumgebungModule`, ihr Store `save_queue`. Die produktive Legacy-Foto-Queue behält nach der Integration ihre bisherigen Dateipfade, die IndexedDB `Erschliessungsumgebung`, deren Version und ihr bisheriges Eintragsformat. Es gibt keine automatische Migration oder Normalisierung zwischen beiden Queues. Beide dürfen den Browser-Lock `erschliessung-save-queue` zur konservativen Serialisierung verwenden; die eigentliche Isolation entsteht durch getrennte Dateien und Datenbanken.

Damit sind Queue-Kollision und globale Write-Freigabe als wichtigste Pre-Merge-Blocker beseitigt. Die Integration von `main` selbst ist damit noch nicht erfolgt.
...
Die Modulkonfiguration legt mit `create.identity_assignment` deklarativ `on_create` oder `reserve_before_create` fest. Bei `on_create` entsteht die verbindliche Identität im finalen Create. Bei `reserve_before_create` reserviert der Server zuvor nur die von den konfigurierten ID-/Signaturstrategien benötigten Identitätswerte. `PUT` vergibt weder ID noch Nummer neu; eine Änderung der Partition ist ohne ausdrückliche Konfigurationsfreigabe `allow_update` gesperrt und bleibt zusätzlich an die Feldrechte gebunden.

Für eine Reservation sendet der Client `operation_id` und den fachlichen Snapshot. Die Runtime leitet relevante Eingaben, etwa das konfigurierte Partitionsfeld, aus der Strategie ab und speichert Anfrage und Ergebnis unter `identity_reservations` im Modul-State. Derselbe Schlüssel mit denselben relevanten Eingaben liefert exakt dieselbe Identität zurück und erhöht keinen Zähler erneut; abweichende relevante Eingaben werden abgelehnt. Die State-Datei wird mit erwartetem Branch-Head atomar committed. Der finale Create muss dieselbe `operation_id` und exakt die reservierte Identität vorlegen, verwendet sie ohne erneute Zählervergabe und schreibt Record, Index und sonstigen State anschließend über die bestehende atomare Commit-Grenze.

Jeder generische API-Create benötigt eine `operation_id`. Beim ersten erfolgreichen Create werden der vollständige fachliche Request, die übergebene reservierte Identität und die erzeugte Record-ID unter `create_operations` im Modul-State zusammen mit Record und Index atomar festgeschrieben. Eine Wiederholung mit identischem Request liest den bereits gespeicherten Record und liefert dessen konkrete Blob-Revision zurück, ohne neue ID-/Signaturvergabe und ohne neuen Commit. Eine Wiederverwendung derselben `operation_id` für einen abweichenden Request wird abgelehnt. Da diese Zuordnung im Repository-State und nicht im Prozessspeicher liegt, überlebt sie Backend-Neustarts.
...
Im Edit-Modus wird `Speichern` nur für einen geöffneten, geänderten Datensatz aktiviert. `FormState` führt dazu drei nicht persistente Zustände: `serverSnapshot` ist der letzte eindeutig bestätigte Record, `workingRecord` der aktuelle Formularstand und `pendingSnapshot` die tiefe Kopie des beim Start eines Requests gesendeten Stands. `dirty` folgt ausschließlich aus dem deterministischen Vergleich der sichtbaren, editierbaren Fachwerte von `workingRecord` und `serverSnapshot`; serververwaltete technische Metadaten beeinflussen ihn nicht. Der generische Payload-Builder liefert aus dem unveränderlichen `pendingSnapshot` ausschließlich diese Fachfelder.

Das Frontend sendet den Snapshot zusammen mit der beim Laden erhaltenen konkreten Record-/Dateirevision als `base_revision` an `PUT /api/modules/{module_key}/records/{record_id}`. Die Repository-Schicht vergleicht sie mit dem Git-Blob-SHA dieser Datei, nicht mit dem Branch-Head. Bei Erfolg wird die bestätigte Serverantwort zum neuen `serverSnapshot`, `meta.revision` zur neuen Basisrevision und `pendingSnapshot` geleert. Änderungen, die während des Requests im Formular entstehen, bleiben im `workingRecord`; danach ergibt der erneute Vergleich entweder `clean` oder weiterhin `dirty`. So kann ohne Reload beliebig oft `dirty → saving → clean/dirty` durchlaufen werden.

Bei Validierungs- und Berechtigungsfehlern, `409`, Netzwerk- oder Serverfehlern bleiben `workingRecord`, `serverSnapshot`, `pendingSnapshot` und die bestätigte Basisrevision unverändert. Der persistierte Queue-Eintrag ist die Grundlage der kontrollierten Wiederaufnahme. Es gibt weiterhin kein Force-Save oder clientseitiges Zusammenführen.

##### Gemeinsame persistente PUT-/Create-Speicherwarteschlange

Updates und Neuanlagen werden vor jedem Remote-Request lokal in der nativen IndexedDB-Datenbank `Erschliessungsumgebung`, Object Store `save_queue`, persistiert. Jeder Eintrag besitzt eine neue `operation_id` (`crypto.randomUUID()`), Modul, Operation, Record-ID, Identität, `base_revision`, den tief kopierten fachlichen Snapshot, FIFO-Sequenz, Zeitstempel, Status, Versuchszähler und strukturierte letzte Fehlerdaten. Beim Update sind Record-ID und konkrete Record-/Dateirevision gesetzt. Beim Create beginnen `record_id`, `identity` und `base_revision` leer; die `operation_id` bleibt von der lokalen Vormerkung über eine mögliche Reservation bis zum finalen POST unverändert. `pendingSnapshot` entspricht dem lokal gesicherten Stand; IndexedDB ist die persistente Quelle, während `FormState` ausschließlich den Zustand des aktuell geöffneten Formulars verwaltet.

Bei Updates und `on_create` lautet der Ablauf `dirty → lokale Transaktion → queued → saving → clean/dirty`. Bei `reserve_before_create` lautet er `dirty → lokale Transaktion → reserving → queued → saving → clean/dirty`. Erst nach erfolgreichem Abschluss der IndexedDB-Transaktion darf der Worker irgendeinen Remote-Request ausführen. Reservation und Create aktualisieren denselben Queue-Eintrag; die bestätigte Identität wird dort gespeichert und unmittelbar in dasselbe noch geöffnete Formular übernommen. Der finale POST sendet exakt diese Identität und dieselbe `operation_id`.

PUT, Reservation und Create teilen eine einzige, über `queue_sequence` geordnete FIFO-Spur. Pro Worker läuft höchstens ein Remote-Vorgang. So wird beispielsweise `Update A → Create B → Update C` exakt in dieser Reihenfolge übertragen; auch die Reservation von B überholt A nicht. Ein Fehler stoppt die Verarbeitung vor allen späteren Einträgen. Für denselben bestehenden Record wird kein zweiter Queue-Eintrag angelegt, solange der erste noch vorhanden ist.

Nach der lokalen Persistierung ist ein Update oder `on_create` navigationssicher. Eine Neuanlage mit `reserve_before_create` bleibt bis zur bestätigten Reservation gegen Navigation geschützt; danach ist ihre finale Identität sichtbar und Navigation erlaubt, auch wenn der Create noch läuft. Nur Änderungen des `workingRecord`, die über den vorgemerkten Snapshot hinausgehen, blockieren weiterhin. Antworten werden über `operation_id`, Modul, Record-ID und die konkrete Form-State-Instanz korreliert und dürfen keinen inzwischen geöffneten anderen Record überschreiben. Nach Create-Erfolg werden der Queue-Snapshot und die Serverantwort bestätigt, die konkrete Blob-Revision übernommen, der Queue-Eintrag entfernt und die Neuanlage sofort in einen normalen `RecordUpdate` überführt. Spätere Formulareingaben bleiben `dirty`; der nächste Save ist ohne Reload ein PUT.

Bei einem IndexedDB-Fehler erfolgt weder Reservation noch POST/PUT; Eingaben bleiben `dirty`. Bei `401/403`, `409`, `422`, 5xx oder Netzwerkfehlern bleiben Queue-Eintrag, Snapshot und eine bereits reservierte Identität erhalten, `attempt_count` und `last_error` werden aktualisiert und die Verarbeitung stoppt ohne automatisches Retry. Nach einer Serverbestätigung wird vor dem Entfernen der Status `confirmed` persistiert; schlägt die lokale Bereinigung fehl, wird der Eintrag nicht blind erneut gesendet.

##### Recovery nach Reload und unklarer Übertragung

Beim Anwendungsstart werden alle vorhandenen PUT- und Create-Einträge geladen, normalisiert und gemeinsam gezählt, aber niemals automatisch gelöscht. Nur eindeutig noch nicht gesendete Einträge werden automatisch freigegeben: `queued` sowie `reserving` ohne Identität. Eine `reserving`-Reservation wird mit derselben `operation_id` wiederholt und ist serverseitig idempotent. Enthält derselbe Eintrag bereits Record-ID und Identität, wird keine zweite Reservation ausgeführt. `saving`, `confirmed`, `auth_error`, `conflict`, `validation_error` und `error` bleiben zunächst sichtbar blockiert.

Ein manuell fortgesetzter unklarer PUT wird vor jedem erneuten Schreiben über die generische Detail-API gelesen. Der fachliche Vergleich verwendet dieselbe rollen- und descriptorbasierte Snapshot-Projektion wie `FormState`; technische Servermetadaten zählen nicht. Entspricht der Serverrecord dem Queue-Snapshot, gilt der PUT mit der gelesenen konkreten Revision als bestätigt und der Eintrag wird ohne zweiten PUT entfernt. Entspricht die Serverrevision noch `base_revision`, wird derselbe Eintrag sicher auf `queued` gesetzt und mit unverändertem Snapshot und Basisstand wiederholt. Eine andere Revision bei anderem Snapshot ergibt dauerhaft `conflict`; es gibt weder Merge noch Überschreibung.

Ein unklarer Create wird bei manueller Fortsetzung mit derselben `operation_id`, demselben Snapshot und derselben reservierten Identität erneut an den idempotenten Create-Endpunkt gesendet. War der erste Commit bereits erfolgreich, liefert der Server den vorhandenen Record zurück. War nur die HTTP-Antwort verloren oder das lokale Entfernen aus IndexedDB fehlgeschlagen, entsteht daher weder ein zweiter Record noch eine zweite Identität.

`401` und `403` führen zu `auth_error`, halten den ersten FIFO-Eintrag samt Snapshot und Identität fest und lösen keinen Redirect aus dem Worker aus. Nach erfolgreicher Anmeldung kann der Benutzer ausdrücklich **Speicherung erneut versuchen** wählen. `409` und `422` werden nicht blind wiederholt. **Lokalen Stand öffnen** lädt Modul, Record-ID beziehungsweise reservierte Identität und Snapshot in das generische Formular; die Queue-Zuordnung bleibt sichtbar. Nach einer Korrektur kann der alte Queue-Eintrag ausdrücklich verworfen und ein neuer normaler Save begonnen werden.

**Lokale Speicherung verwerfen** verlangt eine ausdrückliche Browserbestätigung, entfernt nur den gewählten IndexedDB-Eintrag und löst keine Backend-Operation aus. Danach kann die unveränderte FIFO-Spur mit dem nächsten sicheren Eintrag fortfahren. Es gibt keine Skip-Funktion, keinen automatischen Konflikt-Merge und kein Force-Save. Späte Antworten aktualisieren einen Form State weiterhin nur bei Übereinstimmung von Modul, Record-ID, `operation_id` und konkreter Form-State-Instanz.
...
Das produktive Modul legt Create-Snapshots ausschließlich über die gemeinsame Queue an; es gibt dort keinen direkten konkurrierenden POST-Pfad. Die eigenständige Klasse `RecordCreate` bleibt nur als isolierter Testbaustein für die Transport-/State-Semantik bestehen. Clientseitig werden weder ID noch Signaturnummer, Revision oder technische Metadaten erzeugt.

Nach erfolgreichem POST initialisiert die rollenabhängig gefilterte Serverantwort den bestätigten `serverSnapshot`; `record_id` wird zur geöffneten ID und der konkrete Record-Blob-SHA aus `meta.revision` zur Basisrevision. Ohne zwischenzeitliche weitere Eingabe entspricht der `workingRecord` diesem Stand und ist `clean`; spätere Eingaben bleiben erhalten und machen ihn `dirty`. Der Create-State wechselt damit unmittelbar in den normalen bestehenden Record-/PUT-State, sodass der nächste Save ohne Reload als PUT mit dieser Revision erfolgt. Die URL wird auf die bestätigte Server-ID umgestellt.

Bei `422`, fehlender Berechtigung, abgelaufener Anmeldung, Netzwerk- oder Serverfehlern bleiben Form State, Queue-Snapshot und Dirty State erhalten. Ohne bestätigte Reservation wird keine technische ID oder Signatur angenommen. Die bestehende Dirty-Warnung schützt auch ungespeicherte Create-Entwürfe bei Listen-, Record-, Modul- und Browsernavigation. Vocabulary-Verwaltung, Autosave und Batch-Erfassung sind weiterhin nicht Bestandteil dieses Schritts.
...
Vokabulare sind kleine versionierte YAML- oder JSON-Dateien und entsprechen `schemas/vocabulary.schema.json`. Die stabile Vocabulary-ID steht in `id`; jeder Term besitzt eine stabile `id`, ein veränderbares primäres `label` und `active`. Optional bleiben `description`, `aliases` und `sort_order` erhalten. Sobald mindestens ein Term `sort_order` verwendet, stehen nummerierte Terms deterministisch davor; sonst bleibt die deklarierte Dateireihenfolge maßgeblich. Doppelte Term-IDs, ein Schemafehler, eine fehlende Datei oder eine Abweichung zwischen referenzierter und enthaltener Vocabulary-ID verhindern das Laden der Modulkonfiguration.

Ein Modul referenziert die lokale, beim Start validierte Datei unter `vocabularies.{vocabulary_id}.path`; optional benennt `repository_path` ihren Pfad im Datenrepository, andernfalls wird ein sicherer Pfad aus der lokalen Referenz abgeleitet. Ein `vocabulary_select` nennt ausschließlich die Vocabulary-ID. Der zentrale Vocabulary-Service parst und validiert lokale wie repositorybasierte Inhalte mit demselben Modell, löst `id -> label` auf und stellt aktive sowie alle Terms bereit. Die Repository-Version ist für API und serverseitige Record-Validierung maßgeblich. Für noch nicht migrierte Bestände bleibt die lokale Datei lesbar; eine Termoperation erzeugt eine dort fehlende Repository-Datei jedoch nicht implizit.

Die Rechte stehen deklarativ im Vocabulary-Dokument unter `rights.use`, `rights.add`, `rights.rename` und `rights.deactivate`. Sie werden serverseitig pro Operation gegen die Benutzerrolle geprüft und sind nicht an den Zugriff auf ein einzelnes referenzierendes Modul gekoppelt. `GET /api/vocabularies/{vocabulary_id}` liefert angemeldeten, laut `use` berechtigten Benutzern alle Terms und unter `meta.revision` die konkrete Datei-/Blob-Revision. `POST /api/vocabularies/{vocabulary_id}/terms` fügt eine vom Client mit stabiler ID gelieferte Termdefinition hinzu. `PATCH /api/vocabularies/{vocabulary_id}/terms/{term_id}` ändert entweder nur das Label oder setzt `active` ausschließlich auf `false`. Die schreibenden Routen verlangen den vorhandenen CSRF-Schutz.

Jeder Write liest den aktuellen Repository-Stand, prüft `base_revision` gegen die konkrete Vocabulary-Dateirevision, wendet genau eine Operation an, validiert das vollständige Ergebnis gegen das Vocabulary-Schema und schreibt genau diese Datei über die gemeinsame Repository-Abstraktion. Ein geänderter Vocabulary-Blob liefert `409` ohne Merge, Force-Save oder Überschreiben. Ein konkurrierender Commit an einer anderen Datei kann gegen dieselbe unveränderte Blob-Revision neu angesetzt werden. Erfolg liefert das aktualisierte Vocabulary und dessen neue konkrete Revision; das Branch-Head ist keine Vocabulary-Revision. Ungültige Ergebnisse liefern `422`, unbekannte Vokabulare oder Terms `404` und fehlende Rechte `403`.
...
Presets übernehmen denselben kanonischen `term_ref`. Nach dem Laden wird das jeweils aktuelle Label über die stabile Term-ID aufgelöst, sodass eine spätere Labeländerung weder Record- noch Presetmigration erfordert. Beim Umbenennen bleibt die ID unverändert; beim Deaktivieren bleiben ID, Label und weitere Eigenschaften erhalten. Beide Operationen verändern keinerlei Record-Dateien. Verwendete Terms bleiben mit `active: false` auflösbar. Es gibt keinen normalen Endpunkt zum physischen Löschen. Purge, Konflikt-Merge und Force-Save sind weiterhin nicht implementiert.

Die zentrale Verwaltungsoberfläche unter `/app/vocabularies/` bezieht ihre Liste über `GET /api/vocabularies`; der Katalog entsteht generisch aus der Vocabulary-Registry und liefert nur laut `use` sichtbare Einträge samt benutzerbezogenen Bool-Rechten. Add, Rename und Deactivate werden in der Oberfläche unabhängig voneinander anhand dieser Rechte angeboten, während die serverseitige Prüfung verbindlich bleibt. Der Client hält pro geöffnetem Vocabulary die konkrete `meta.revision`, sendet sie als `base_revision` und übernimmt eine neue Revision ausschließlich aus einer bestätigten Write-Antwort. Damit sind mehrere aufeinanderfolgende Änderungen ohne Browser-Reload möglich.

Bei `409` bleibt die lokale Eingabe erhalten, der Stand gilt nicht als gespeichert und kann über „Vocabulary neu laden“ nach ausdrücklicher Bestätigung durch den aktuellen Serverstand ersetzt werden; es gibt keinen Merge oder Force-Save. `422`, doppelte IDs, `403` und `404` werden als verständliche Fehler angezeigt, ohne die lokale Revision künstlich fortzuschreiben. Deactivate erfordert eine explizite Bestätigung und lässt den inaktiven Term sichtbar. Die Oberfläche besitzt weder Delete-/Reactivate-Aktion noch Offline-Queue.
