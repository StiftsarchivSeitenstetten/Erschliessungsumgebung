...
export function createIndexedDbSaveQueueStore({
  indexedDB,
  ...
  async function list() {
    ...
    await transactionDone(transaction);
    return result.map(cloneValue).sort((left, right) => (
      (left.queue_sequence ?? Number.MAX_SAFE_INTEGER) - (right.queue_sequence ?? Number.MAX_SAFE_INTEGER) ||
      left.created_at.localeCompare(right.created_at) || left.operation_id.localeCompare(right.operation_id)
    ));
  }
  ...
  return {databaseName, databaseVersion, storeName, open, put, get, list, update, remove};
}
